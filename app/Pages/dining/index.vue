...
const { data: dining } = await useAsyncData('dining', () =>
  $api('/get-dinings')
);
</script>
...
<template>
  ...
  <section class="relative h-[400px] lg:h-[500px] overflow-hidden group">
    <!-- Background -->
    <img src="/images/dining.webp" alt="Tech products banner" class="absolute inset-0 w-full h-full object-cover transition-transform duration-1000 group-hover:scale-105" loading="eager" />
    <!-- Gradient Overlay -->
    ...
    <div class="container mx-auto flex flex-wrap lg:py-5">
      ...
      <div class="w-full rounded-lg px-4">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          <DiningCard v-for="dining in dining?.data" :key="dining.id" :dining="dining" />
        </div>
      </div>
    </div>
  </section>
