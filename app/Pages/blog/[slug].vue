            ...
            <div class="flex items-center gap-3 mb-6">
              ...
              <div class="flex items-center gap-1">
                <template v-for="i in 5" :key="i">
                  <Icon v-if="i <= Math.floor(Math.round(blogData?.blog_review * 2) / 2)"
                    name="material-symbols:star-rounded" class="w-4 h-4 text-yellow-400" />
                  <Icon v-else-if="i - 0.5 === Math.round(blogData?.blog_review * 2) / 2"
        ...
        <div class="flex flex-col lg:flex-row gap-12">
            ...
            <article class="bg-white shadow-sm border border-gray-100 p-8 lg:p-12">
              ...
              <div class="border-t border-gray-200 pt-12">
                ...
                <div v-if="blogData.reviews?.length" class="space-y-6">
                  <div v-for="rev in blogData.reviews" :key="rev.id" class="bg-gray-50 p-6 border border-gray-200">
                    <!-- Header -->
              ...
              <div class="border-t border-gray-200 pt-12 mt-12">
                ...
                <div class="flex gap-2 mb-6">
                  <button v-for="star in 5" :key="star" @click="rating = star" @mouseenter="hoverRating = star"
                    @mouseleave="hoverRating = 0" class="text-3xl transition-colors"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                  <input v-model="review.name" type="text" placeholder="Name"
                    class="w-full p-4 border border-gray-200 focus:border-primary outline-none" />
                  <input v-model="review.email" type="email" placeholder="Email"
                    class="w-full p-4 border border-gray-200 focus:border-primary outline-none" />
                </div>
                <textarea v-model="review.message" rows="4" placeholder="Your review..."
                  class="w-full p-4 border border-gray-200 focus:border-primary outline-none mb-6"></textarea>

                <button @click="onSubmit"
                  class="bg-primary text-white px-8 py-4 font-bold hover:bg-primary-dark transition-all">
                  Submit Review
                </button>
              </div>
            </article>
          ...
          <div class="w-full lg:w-1/3 space-y-8">
            <div class="bg-white border border-gray-100 p-6">
              ...
              <div class="space-y-6">
                <NuxtLink v-for="item in trendingBlogs" :key="item.id" :to="`/blog/${item.slug}`"
            ...
            <div class="bg-white border border-gray-100 p-6">
              ...
              <div class="space-y-6">
                <NuxtLink v-for="item in latestBlogs" :key="item.id" :to="`/blog/${item.slug}`"
                  class="flex gap-4 group">
                  <img :src="item.thumbnail" class="w-20 h-20 object-cover rounded shadow-sm" />
                  <div class="flex-1">
                    <h4 class="font-bold group-hover:text-primary transition-colors line-clamp-2">{{ item.title }}</h4>
